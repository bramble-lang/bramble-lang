...
pub fn resolve_types_with_imports(
    ...
    let mut sm_asts = vec![];
    for t in ast {
        let mut sm_ast = sa.from_module(t, trace_semantic_node);
        SymbolTable::add_item_defs_to_table(&mut sm_ast)?;
        sm_asts.push(sm_ast);
    }

    let mut semantic = TypeResolver::new(&sm_asts[0]);
    ...
    pub fn test_identifiers() {
        for (text, expected) in vec![
            ...
            let tokens: Vec<Token> = Lexer::new(&text)
                ...
                .collect::<Result<_>>()
                .unwrap();
            let ast = vec![parser::parse(&tokens).expect(&format!("{}", text)).unwrap()];
    ...
    pub fn test_path_to_function() {
        for (text, expected) in vec![
            ...
            let tokens: Vec<Token> = Lexer::new(&text)
                ...
                .collect::<Result<_>>()
                .unwrap();
            let ast = vec![parser::parse(&tokens).unwrap().unwrap()];
