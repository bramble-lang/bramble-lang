        ...
        match stream.next_if(&Lex::ModuleDef) {
            Some(module) => match stream.next_if_id() {
                Some((module_name, _, _)) => {
                    ...
                    stream.next_must_be(&Lex::LBrace)?; // TRACE error
    ...
    fn parse_items(
        ...
        while stream.peek().is_some() {
            ...
            if let Some(f) = self.function_def(stream)? {
                items.push(Item::Routine(f)); // TRACE?
            }
    ...
    fn extern_def(&self, stream: &mut TokenStream) -> ParserResult<Extern<ParserContext>> {
        match stream.next_if(&Lex::Extern) {
            Some(extern_tok) => match self.function_decl(stream, true)? {
                ...
                None => err!(extern_tok.span(), ParserError::ExternExpectedFnDecl).map_err(|err| {
                    self.logger.write(Event::<ParserError> {
                        stage: "parser",
                        input: err.span(),
                        msg: Err(&err),
        ...
        match stream.next_if(&Lex::Struct) {
            Some(st_def) => match stream.next_if_id() {
                Some((id, _, _)) => {
                    ...
                    let ctx = stream
                        .next_must_be(&Lex::RBrace)? // TRACE
                        .to_ctx()
                        .join(st_def.to_ctx());
                    Ok(Some(StructDef::new(id, ctx, fields))) // TRACE
                }
                None => {
                    // TRACE
    ...
    fn consume_type(&self, stream: &mut TokenStream) -> ParserResult<(Type, ParserContext)> {
        ...
        let ty = match stream.next_if(&Lex::Primitive(Primitive::U8)) {
            ...
            _ => match self.path(stream)? {
                Some((path, path_ctx)) => Some((Type::Custom(path), path_ctx)).map(|v| {
                    self.logger.write(Event::<ParserError> {
                        stage: "parser",
                        input: v.1.span(),
                        msg: Ok("Custom Type"),
        ...
        match stream.next_if(&Lex::LBracket) {
            Some(lbracket) => {
                ...
                let len = match len {
                    ...
                    _ => return err!(len.span(), ParserError::ArrayExpectedIntLiteral), // TRACE
                };

                let ctx = stream.next_must_be(&Lex::RBracket)?.to_ctx().join(ctx);
                Ok(Some((Type::Array(Box::new(element_ty), len), ctx))) // TRACE
                    .map(|ok| {
                        ok.map(|v| {
                            self.logger.write(Event::<ParserError> {
                                stage: "parser",
