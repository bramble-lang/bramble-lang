    ...
    fn module(&self, stream: &mut TokenStream) -> ParserResult<Module<ParserContext>> {
        match stream.next_if(&Lex::ModuleDef) {
            Some(module) => match stream.next_if_id() {
                ...
                _ => err!(module.span(), ParserError::ModExpectedName).map_err(|err| {
                    self.logger.write(Event::<ParserError> {
                        stage: "parser",
                        input: err.span(),
                        msg: Err(&err),
            ...
            Some(extern_tok) => match self.function_decl(stream, true)? {
                Some((fn_ctx, fn_name, params, has_varargs, fn_type)) => {
                    if has_varargs && params.len() == 0 {
                        return err!(fn_ctx.span(), ParserError::ExternInvalidVarArgs).view_err(
                            |err| {
                                self.logger.write(Event::<ParserError> {
                                    stage: "parser",
                                    input: err.span(),
                    ...
                    .map(|ok| {
                        ok.view(|v| {
                            self.logger.write(Event::<ParserError> {
                                stage: "parser",
                                input: v.span(),
                                msg: Ok("Extern Definition"),
                            });
                        })
                    })
                }
                None => {
                    err!(extern_tok.span(), ParserError::ExternExpectedFnDecl).view_err(|err| {
                        self.logger.write(Event::<ParserError> {
                            stage: "parser",
                            input: err.span(),
                            msg: Err(&err),
    ...
    fn function_def(&self, stream: &mut TokenStream) -> ParserResult<RoutineDef<ParserContext>> {
        let (fn_ctx, fn_name, params, fn_type) = match self.function_decl(stream, false)? {
            Some((ctx, name, params, is_variadic, ret_ty)) => {
                if is_variadic {
                    return err!(ctx.span(), ParserError::FnVarArgsNotAllowed).view_err(|err| {
                        self.logger.write(Event::<ParserError> {
                            stage: "parser",
                            input: err.span(),
                            msg: Err(&err),
    ...
    fn consume_type(&self, stream: &mut TokenStream) -> ParserResult<(Type, ParserContext)> {
        ...
        let ty = match stream.next_if(&Lex::Primitive(Primitive::U8)) {
            Some(Token {
                ...
                ty.map(|ty| (ty, ctx)).view(|v| {
                    self.logger.write(Event::<ParserError> {
                        stage: "parser",
                        input: v.1.span(),
                        msg: Ok("Primitive Type"),
                    });
                })
            }
            _ => match self.path(stream)? {
                Some((path, path_ctx)) => Some((Type::Custom(path), path_ctx)).view(|v| {
                    self.logger.write(Event::<ParserError> {
                        stage: "parser",
                        input: v.1.span(),
                        msg: Ok("Custom Type"),
    ...
    fn array_type(&self, stream: &mut TokenStream) -> ParserResult<(Type, ParserContext)> {
        ...
        match stream.next_if(&Lex::LBracket) {
            Some(lbracket) => {
                ...
                Ok(Some((Type::Array(Box::new(element_ty), len), ctx))).view(|v| {
                    self.logger.write(Event::<ParserError> {
                        stage: "parser",
                        input: v.1.span(),
                        msg: Ok("Array Type"),
    ...
    pub(super) fn id_declaration(
        ...
        match stream.next_ifn(vec![Lex::Identifier(StringId::new()), Lex::Colon]) {
            Some(decl_tok) => {
                ...
                Ok(Some(Expression::IdentifierDeclare(ctx, id, ty))).view(|v| {
                    self.logger.write(Event::<ParserError> {
                        stage: "parser",
                        input: v.span(),
