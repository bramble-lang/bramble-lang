...
fn main() {
    ...
    for (path, tokens) in &token_sets {
        if let Some((name, parent_path)) = path.split_last() {
            // from the root traverse down parent_path until the parent module of this file is found
            // creating any missing modules along the way
            let parent = if parent_path.len() == 0 {
                &mut root
            } else {
                find_or_create_module(&mut root, parent_path).unwrap()
            };
            match parser::parser::parse(name, tokens) {
                Ok(Some(ast)) => parent.add_module(ast),
                Ok(None) => {
