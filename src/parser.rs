    ...
    fn if_expression(iter: &mut TokenIter) -> PResult {
        Ok(match iter.peek() {
            Some(Token { l, s: Lex::If }) => {
                ...
                // expression
                let cond =
                    expression(iter)?.ok_or("Expected conditional expressoin after if")?;
                // lbrace
                iter.next()
                ...
                // expression
                let true_arm =
                    expression(iter)?.ok_or("Expression in true arm of if expression")?;
                // rbrace
                iter.next()
