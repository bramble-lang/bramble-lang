...
run() {
    ...
    if [[ "$OSTYPE" == "linux-gnu"* ]]; then
        ...
        echo "Compiling"
        cargo run -- -p machos --input "./src/import/shared" -o ./target/shared.obj --manifest
        cargo run -- -p machos --input "../braid/std" -o ./target/std.obj --manifest
        cargo run -- -p machos --import ./target/std.manifest,./target/shared.manifest "$@" -o ./target/output.obj
        echo ""
        echo "Assembling"
        nasm -g -f macho64 ../braid/macho64/llvm/std/input.asm -l ./target/std_input.lst -o ./target/std_input.obj > assembler.log
        gcc -w ./target/shared.obj ./target/std.obj ./target/std_input.obj ./target/output.obj -g -o ./target/output -m64 2>&1 > gcc.log
        built=1
    else
