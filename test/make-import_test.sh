...
run() {
	...
	if [[ $OSTYPE == "linux-gnu"* ]]; then
		echo "Compiling"
		../target/debug/bramblec --mir-beta -p linux --input "./import/shared" -o ./target/shared.obj --manifest
		../target/debug/bramblec --mir-beta -p linux --import ./target/shared.manifest --input "./import/indirect" -o ./target/indirect.obj --manifest
		../target/debug/bramblec --mir-beta -p linux --input "../bramble/std" -o ./target/std.obj --manifest
		../target/debug/bramblec --mir-beta -p linux --import ./target/std.manifest,./target/shared.manifest,./target/indirect.manifest "$@" -o ./target/output.obj
		echo ""
		echo "Assembling"
